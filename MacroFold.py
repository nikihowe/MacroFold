...
import subprocess
from scripts.get_all_structures import get_structs, make_ct, get_match
...
def loadDangle3Energy(no_dangle=False, zero_energies=False):
    ...
    dangle3Energy = np.full((4,4,4), 1, dtype=np.float64)
    if no_dangle:
    ...
    with open("./data/{}".format(filename)) as csvfile:
        ...
        for i, row in enumerate(reader):
            ...
            if i == 0:
                ...
                dangle3Energy[base_dict[row[0]],base_dict[row[1]],base_dict[row[2]]] = math.exp(-beta*float(row[3]))
    #print("d3", len(dangle3Energy))
...
def loadDangle5Energy(no_dangle=False, zero_energies=False):
    ...
    dangle5Energy = np.full((4,4,4), 1, dtype=np.float64)
    if no_dangle:
    ...
    with open("./data/{}".format(filename)) as csvfile:
        ...
        for i, row in enumerate(reader):
            ...
            if i == 0:
                ...
                dangle5Energy[base_dict[row[0]],base_dict[row[1]],base_dict[row[2]]] = math.exp(-beta*float(row[3]))
...
def loadStackEnergy(zero_energies=False):
    global stackEnergy
    stackEnergy = np.full((4,4,4,4), 1, dtype=np.float64)
    if zero_energies:
    ...
    with open("./data/{}".format(filename)) as csvfile:
        ...
        for i, row in enumerate(reader):
            ...
            if i == 0:
                ...
                stackEnergy[base_dict[row[0]],base_dict[row[1]],base_dict[row[2]],base_dict[row[3]]] = math.exp(-beta*float(row[4]))
...
def loadTerminalMismatchHEnergy(no_dangle=False, zero_energies=False):
    ...
    terminalMismatchHEnergy = np.full((4,4,4,4), 1, dtype=np.float64)
    if no_dangle:
    ...
    with open("./data/{}".format(filename)) as csvfile:
        ...
        for i, row in enumerate(reader):
            ...
            if i == 0:
                ...
                terminalMismatchHEnergy[base_dict[row[0]],base_dict[row[1]],base_dict[row[2]],base_dict[row[3]]] = math.exp(-beta*float(row[4]))

def loadTerminalMismatchEEnergy(no_dangle=False, zero_energies=False):
    global terminalMismatchEEnergy
    terminalMismatchEEnergy = np.full((4,4,4,4), 1, dtype=np.float64)
    if no_dangle:
    ...
    with open("./data/{}".format(filename)) as csvfile:
        ...
        for i, row in enumerate(reader):
            ...
            if i == 0:
                ...
                terminalMismatchEEnergy[base_dict[row[0]],base_dict[row[1]],base_dict[row[2]],base_dict[row[3]]] = math.exp(-beta*float(row[4]))
                
def loadTerminalMismatchMEnergy(no_dangle=False, zero_energies=False):
    global terminalMismatchMEnergy
    terminalMismatchMEnergy = np.full((4,4,4,4), 1, dtype=np.float64)
    if no_dangle:
    ...
    with open("./data/{}".format(filename)) as csvfile:
        ...
        for i, row in enumerate(reader):
            ...
            if i == 0:
                ...
                terminalMismatchMEnergy[base_dict[row[0]],base_dict[row[1]],base_dict[row[2]],base_dict[row[3]]] = math.exp(-beta*float(row[4]))
...
def loadInt11(zero_energies=False):
    global int11
    int11 = np.full((4,4,4,4,4,4), 1, dtype=np.float64)
    if zero_energies:
    ...
    with open("./data/{}".format(filename)) as csvfile:
        ...
        for i, row in enumerate(reader):
            ...
            if i == 0:
                ...
                int11[base_dict[row[0]],base_dict[row[1]],base_dict[row[2]],
                      base_dict[row[3]],base_dict[row[4]],base_dict[row[5]]] = math.exp(-beta*float(row[6]))

# Fill the internal 2x1 bulge table (sequence : energy)
def loadInt21(zero_energies=False):
    global int21
    int21 = np.full((4,4,4,4,4,4,4), 1, dtype=np.float64)
    if zero_energies:
    ...
    with open("./data/{}".format(filename)) as csvfile:
        ...
        for i, row in enumerate(reader):
            ...
            if i == 0:
                ...
                int21[base_dict[row[0]],base_dict[row[1]],base_dict[row[2]],
                      base_dict[row[3]],base_dict[row[4]],base_dict[row[5]],base_dict[row[6]]] = math.exp(-beta*float(row[7]))

# Fill the 2x2 internal bulge tables (sequence : energy)
def loadInt22(zero_energies=False):
    global int22
    int22 = np.full((4,4,4,4,4,4,4,4), 1, dtype=np.float64)
    if zero_energies:
    ...
    with open("./data/{}".format(filename)) as csvfile:
        ...
        for i, row in enumerate(reader):
            ...
            if i == 0:
                ...
                int22[base_dict[row[0]],base_dict[row[1]],base_dict[row[2]],base_dict[row[3]],
                      base_dict[row[4]],base_dict[row[5]],base_dict[row[6]],base_dict[row[7]]] = math.exp(-beta*float(row[8]))
...
def load_penalty_dicts(zero_energies=False):
    ...
    if zero_energies:
        endPenalty = {(0,3): 1, (3,0): 1, (2,3): 1, (3,2): 1}
        loopEndPenalty = {(0,3): 1, (3,0): 1, (2,3): 1, (3,2): 1}
        loopFirstMismatchEnergy = {(3,3): 1, (2,0): 1, (0,2): 1}
        guClosureEnergy = {(2,2,2,3): 1}
    else:
        endPenalty = {(0,3): math.exp(-beta*0.5), (3,0): math.exp(-beta*0.5), (2,3): math.exp(-beta*0.5), (3,2): math.exp(-beta*0.5)}
        loopEndPenalty = {(0,3): math.exp(-beta*0.7), (3,0): math.exp(-beta*0.7), (2,3): math.exp(-beta*0.7), (3,2): math.exp(-beta*0.7)}
        loopFirstMismatchEnergy = {(3,3): math.exp(-beta*(-0.7)), (2,0): math.exp(-beta*(-1.1)), (0,2): math.exp(-beta*(-1.1))}
        guClosureEnergy = {(2,2,2,3): math.exp(-beta*(-2.2))}
...
global allowedPairs
allowedPairs = {(0,3), (3,0), (2,1), (1,2), (2,3), (3,2)}
    ...
    # Otherwise, load all legal pairs
        ...
        for i in range(N):
            ...
            for j in range(min(i+4, N), max(N, i+N-3)):
                if ( (t[i] in int_dict and t[j] in int_dict) and
                     (t[i] + t[j] == 3 or t[i] + t[j] == 5) ):
                    R[-1].append(j)
...
def singleStemTermF(i, j):
    ...
    # No dangle
    for k in R[i]:
        endPen = endPenalty.get((t[i],t[k]), 1)
    ...
    for k in R[i]:
        ...
        scale_num = j-k
        energy_sum += (Zb[i, k]
                      * dangle3Energy[t[i],t[k],t[k+1]]
                      * (eb**(j-k))
                      * endPen)
    # 5'D dangle
    for k in R[i+1]:
        endPen = endPenalty.get((t[i+1],t[k]), 1)
        if k > min(j, N-1): break
        scale_num = j-k+1
        energy_sum += (dangle5Energy[t[i],t[i+1],t[k]]
                      * Zb[i+1, k]
                      * (eb**(j-k+1))
                      * endPen)
    # terminal dangle
    ...
    for k in R[i+1]:
        ...
        if not (t[i], t[k+1]) in allowedPairs:
            energy_sum += (terminalMismatchMEnergy[t[k],t[k+1],t[i],t[i+1]]
                          * Zb[i+1, k]
                          * (eb**(j-k+1))
                          * endPen)
...
def additionalStemTermF(i, j):
    ...
    for k in R[i+1]:
        ...
        if not (t[i], t[k+1]) in allowedPairs:
            energy_sum += (eb*eb
                          * terminalMismatchMEnergy[t[k],t[k+1],t[i],t[i+1]]
                          * Zb[i+1, k]
                          * Z1[k+2, j]
                          * endPen)
...
def fillZ3():
    ...
    for i in reversed(range(N-4)):
        ...
        for k in R[i]:
            ...
            endPen = endPenalty.get((t[i],t[k]), 1)
            energy_sum += Zb[i, k]*dangle3Energy[t[i],t[k],t[k+1]]*Z3[k+2]*endPen
            #print("  multiplying:")
            #try:
                #print("  Zb[i, k]: {}".format(-1/beta*math.log(Zb[i, k])))
            #except:
                #print("not allowed to pair")
            #print("  3'dangle: {}".format(dangle3Energy[t[i],t[k],t[k+1]]))
        ...
        for k in R[i+1]:
            ...
            endPen = endPenalty.get((t[i+1],t[k]), 1)
            energy_sum += Zb[i+1, k]*dangle5Energy[t[i],t[i+1],t[k]]*Z3[k+1]*endPen
            #print("  multiplying:")
            #try:
                #print("  Zb[i+1, k]: {}".format(-1/beta*math.log(Zb[i+1, k])))
            #except:
                #print("not allowed to pair")
            #print("  5'dangle: {}".format(dangle5Energy[t[i],t[i+1],t[k]]))
            #print("  Z[k+1]:   {}".format(-1/beta*math.log(Z3[k+1])))
            #print("  EndPen:   {}".format(-1/beta*math.log(endPen)))
        ...
        for k in R[i+1]:
            if k > N-3: break
            #print("  Looking at {}: {}".format(k, s[:i]+'('+s[i]+'['+s[i+1]+']'+s[i+2:k]+'['+s[k]+']'+s[k+1:]+')'))
            #if s[k]+s[k+1]+s[i]+s[i+1] in terminalMismatchEEnergy: changed on 10 Jan
            ...
            if not (t[i],t[k+1]) in allowedPairs:
                ...
                endPen = endPenalty.get((t[i+1],t[k]), 1)
                energy_sum += (Zb[i+1, k]
                              * terminalMismatchEEnergy[t[k],t[k+1],t[i],t[i+1]]
                              * Z3[k+2]
                              * endPen)
            else:
            ...
            #except:
                #print("not allowed to pair")
            #print("   Z[k+2]:     {}".format(-1/beta*math.log(Z3[k+2])))
            #print("   Ddangle:    {}".format(terminalMismatchEEnergy.get(s[k]+s[k+1]+s[i]+s[i+1], "Not Allowed")))
            #print("   EndPen:     {}".format(-1/beta*math.log(endPen)))
        ...
        if N-2 in R[i]:
            # no dangle
            endPen = endPenalty.get((t[i],t[N-2]), 1)
            energy_sum += Zb[i, N-2]*endPen
            #print("  Zb[i, N-2]:  {}".format(-1/beta*math.log(Zb[i, N-2])))
            #print("  EndPen:      {}".format(-1/beta*math.log(endPen)))

            # 3'D
            energy_sum += Zb[i, N-2]*dangle3Energy[t[i],t[N-2],t[N-1]]*endPen
            #print("  Zb[i, N-2]:  {}".format(-1/beta*math.log(Zb[i, N-2])))
            #print("  3dangle:     {}".format(-1/beta*math.log(dangle3Energy[t[i],t[N-2],t[N-1]])))
            #print("  EndPen:      {}".format(-1/beta*math.log(endPen)))

        if N-2 in R[i+1]:
            # 5'D
            endPen = endPenalty.get((t[i+1],t[N-2]), 1)
            energy_sum += Zb[i+1, N-2]*dangle5Energy[t[i],t[i+1],t[N-2]]*endPen
            #print("  Zb[i+1, N-2]:  {}".format(-1/beta*math.log(Zb[i+1, N-2])))
            #print("  5dangle:       {}".format(-1/beta*math.log(dangle5Energy[t[i],t[i+1],t[N-2]])))
            #print("  EndPen:        {}".format(-1/beta*math.log(endPen)))

            # tstacke
            #if s[N-2]+s[N-1]+s[i]+s[i+1] in terminalMismatchEEnergy: # changed on 10 Jan
            # important because terminalMismatchEEnergy contains GU pairs, which we don't want to consider
            if not (t[i], t[N-1]) in allowedPairs:
                # ^^ we could replace this with simply checking if (s[k+1], s[i]) is a WC pair. We can check in the 'allowedPairs' list.
                # ^^ done
                # Use the same endPenalty as before, because we are still looking at (i, N-2)
                energy_sum += Zb[i+1, N-2]*terminalMismatchEEnergy[t[N-2],t[N-1],t[i],t[i+1]]*endPen
                #print("  Zb[i+1, N-2]:  {}".format(-1/beta*math.log(Zb[i+1, N-2])))
                #print("  tstack:        {}".format(-1/beta*math.log(terminalMismatchEEnergy[t[N-2],t[N-1],t[i],t[i+1]])))
                #print("  EndPen:        {}".format(-1/beta*math.log(endPen)))

        # Case where k = N-1 -- here we only have the no dangle and the 5'D cases
        if N-1 in R[i]:
            # No dangle
            endPen = endPenalty.get((t[i],t[N-1]), 1)
        ...
        if N-1 in R[i+1]:
            # 5'D
            endPen = endPenalty.get((t[i+1],t[N-1]), 1) # changed N-2 to N-1 on Jan 10. this was a bug.
            energy_sum += Zb[i+1, N-1]*dangle5Energy[t[i],t[i+1],t[N-1]]*endPen
            #print("  Zb[i+1, N-1]:  {}".format(-1/beta*math.log(Zb[i+1, N-1])))
            #print("  5dangle:       {}".format(-1/beta*math.log(dangle5Energy[t[i],t[i+1],t[N-1]])))
            #print("  EndPen:        {}".format(-1/beta*math.log(endPen)))
...
def fillZ5():
    ...
    for j in range(4, N):
        ...
        for k in L[j]:
            if k < 2: break
            endPen = endPenalty.get((t[k],t[j]), 1)
            energy_sum += Zb[k, j]*Z5[k-1]*endPen
        # 3'D
        for k in L[j-1]:
            if k < 2: break
            endPen = endPenalty.get((t[k],t[j-1]), 1)
            energy_sum += Zb[k, j-1]*dangle3Energy[t[k],t[j-1],t[j]]*Z5[k-1]*endPen
        # 5'D
        for k in L[j]:
            if k < 2: break
            endPen = endPenalty.get((t[k],t[j]), 1)
            energy_sum += Zb[k, j]*dangle5Energy[t[k-1],t[k],t[j]]*Z5[k-2]*endPen
        # ts
        for k in L[j-1]:
            if k < 2: break
            #if s[j-1]+s[j]+s[k-1]+s[k] in terminalMismatchEEnergy:
            if not (t[k-1], t[j]) in allowedPairs: # done on Jan 10
                endPen = endPenalty.get((t[k],t[j-1]), 1)
                energy_sum += (Zb[k, j-1]
                              * terminalMismatchEEnergy[t[j-1],t[j],t[k-1],t[k]]
                              * Z5[k-2]
                              * endPen)
            else:
                energy_sum += 0
        ...
        if 1 in L[j]:
            endPen = endPenalty.get((t[1],t[j]), 1)
            energy_sum += Zb[1, j]*endPen + Zb[1, j]*dangle5Energy[t[0],t[1],t[j]]*endPen

        # 3'D
        if 1 in L[j-1]:
            endPen = endPenalty.get((t[1],t[j-1]), 1)
            energy_sum += Zb[1, j-1]*dangle3Energy[t[1],t[j-1],t[j]]*endPen 
            # tstacke
            #if s[j-1]+s[j]+s[0]+s[1] in terminalMismatchEEnergy:
            if not (t[0], t[j]) in allowedPairs: # done on Jan 10
                # Use the same endPenalty as before, because we are still looking at (1, j-1)
                energy_sum += Zb[1, j-1]*terminalMismatchEEnergy[t[j-1],t[j],t[0],t[1]]*endPen

        # Case where k = 0 -- here we only have the no dangle and the 3'D cases
        # No dangle
        if 0 in L[j]:
            endPen = endPenalty.get((t[0],t[j]), 1)
            energy_sum += Zb[0, j]*endPen

        # 3'D
        if 0 in L[j-1]:
            endPen = endPenalty.get((t[0],t[j-1]), 1)
            energy_sum += Zb[0, j-1]*dangle3Energy[t[0],t[j-1],t[j]]*endPen
...
def traceZ3(i):
    ...
    for k in R[i]:
        ...
        endPen = endPenalty.get((t[i],t[k]), 1)
        prob1 = Zb[i, k]*dangle3Energy[t[i],t[k],t[k+1]]*Z3[k+2]*endPen/Z3[i]
    ...
    for k in R[i+1]:
        if k > N-3: break
        endPen = endPenalty.get((t[i+1],t[k]), 1)
        prob1 = Zb[i+1, k]*dangle5Energy[t[i],t[i+1],t[k]]*Z3[k+1]*endPen/Z3[i]
    ...
    for k in R[i+1]:
        ...
        #if s[k]+s[k+1]+s[i]+s[i+1] in terminalMismatchEEnergy:
        if not (t[i], t[k+1]) in allowedPairs: # done on Jan 10
            endPen = endPenalty.get((t[i+1],t[k]), 1) # THIS WAS NOT HERE BEFORE, ADDED FEB 19
            prob1 = Zb[i+1, k]*terminalMismatchEEnergy[t[k],t[k+1],t[i],t[i+1]]*Z3[k+2]*endPen/Z3[i]
        else:
    ...
    if N-2 in R[i]:
        ...
        prob1 = Zb[i, N-2]*dangle3Energy[t[i],t[N-2],t[N-1]]*endPen/Z3[i]
    ...
    if N-2 in R[i+1]:
        endPen = endPenalty.get((t[i+1],t[N-2]), 1)
        prob1 = Zb[i+1, N-2]*dangle5Energy[t[i],t[i+1],t[N-2]]*endPen/Z3[i]
        ...
        # tstacke
        #if s[N-2]+s[N-1]+s[i]+s[i+1] in terminalMismatchEEnergy:
        if not (t[i], t[N-1]) in allowedPairs: # done on Jan 10
            # ^^ we could replace this with simply checking if (s[k+1], s[i]) is a WC pair. We can check in the 'allowedPairs' list.
            # Use the same endPenalty as before, because we are still looking at (i, N-2)
            prob1 = Zb[i+1, N-2]*terminalMismatchEEnergy[t[N-2],t[N-1],t[i],t[i+1]]*endPen/Z3[i]
        else:
            prob1 = 0
    ...
    # 5'D
    if N-1 in R[i+1]:
        endPen = endPenalty.get((t[i+1],t[N-1]), 1) # fixed on Jan 10 2017. this was a bug.
        prob1 = Zb[i+1, N-1]*dangle5Energy[t[i],t[i+1],t[N-1]]*endPen/Z3[i]
...
def traceZb(i, j):
        ...
        while j-e+d-i < l+2 and eIdx >= 0:
            ...
            # Bulge situation
                ...
                if top > 1 or bottom > 1:
                    energyZ *= endPenalty.get((t[i],t[j]), 1)
                    energyZ *= endPenalty.get((t[e],t[d]), 1)
                # Stacking energy (added 5 Dec 2016)
                if top == 1 or bottom == 1:
                    energyZ *= stackEnergy[t[i],t[d],t[e],t[j]]
            # 1x1 situation
            elif top == 1 and bottom == 1:
                energyZ *= int11[t[i],t[i+1],t[d],t[e],t[j-1],t[j]]
            # 2x1 situation
            elif top == 2 and bottom == 1:
                energyZ *= int21[t[e],t[j-1],t[j],t[i],t[i+1],t[i+2],t[d]]
            # 1x2 situation
            elif top == 1 and bottom == 2:
                energyZ *= int21[t[i],t[i+1],t[d],t[e],t[j-2],t[j-1],t[j]]
            # 2x2 situation
            elif top == 2 and bottom == 2:
                energyZ *= int22[t[i],t[i+1],t[i+2],t[d],t[e],t[j-2],t[j-1],t[j]]
            # General case
            # See http://rna.urmc.rochester.edu/NNDB/turner99/internal.html
                ...
                if top > 1 and bottom > 1:
                    # 5' side
                    energyZ *= loopFirstMismatchEnergy.get((t[i+1],t[j-1]), 1)
                    # 3' sid
                    energyZ *= loopFirstMismatchEnergy.get((t[e+1],t[d-1]), 1)
      
                # Loop closure penalty term
                energyZ *= loopEndPenalty.get((t[i],t[j]), 1)
                energyZ *= loopEndPenalty.get((t[e],t[d]), 1)
    ...
    # 3'D
    prob1 = ea*eb*ec*dangle3Energy[t[j],t[i],t[i+1]]*Z2[i+2, j-1]/Zb[i, j]
    ...
    # 5'D
    prob1 = ea*eb*ec*dangle5Energy[t[j-1],t[j],t[i]]*Z2[i+1, j-2]/Zb[i, j]
    ...
    if not (t[i+1], t[j-1]) in allowedPairs:
        prob1 = (ea*eb*eb*ec*terminalMismatchMEnergy[t[i],t[i+1],t[j-1],t[j]]
                * Z2[i+2, j-2])/Zb[i, j]
    ...
    if i in pair_dict:
        ...
        # 3'D
            if k in dangle_list:
                if debug_energy: 
                    ...
                    print("Z3 dangle3 {},{},{}:".format(i, k, k+1), (-1/beta)*math.log(dangle3Energy[t[i],t[k],t[k+1]]))
                return (traceZbd(i)
                       * dangle3Energy[t[i],t[k],t[k+1]]
                       * traceZ3d(k+2)
                       * endPen)
        ...
        # 3'D 
            if N-2 in dangle_list:
                if debug_energy:
                    print("Z3 endPen {},{}:".format(i, N-2), (-1/beta)*math.log(endPen))
                    print("Z3 dangle3 {},{},{}:".format(i, N-2, N-1), (-1/beta)*math.log(dangle3Energy[t[i],t[N-2],t[N-1]]))
                return traceZbd(i)*dangle3Energy[t[i],t[N-2],t[N-1]]*endPen
        ...
        if k <= N-3:
            ...
            if not k in dangle_list:
                if debug_energy: 
                    ...
                    print("Z3 dangle5 {},{},{}:".format(i, i+1, k), (-1/beta)*math.log(dangle5Energy[t[i],t[i+1],t[k]]))
                return traceZbd(i+1)*dangle5Energy[t[i],t[i+1],t[k]]*traceZ3d(k+1)*endPen
            
            # ts
            if k in dangle_list:
                #if any(k+1 in pair for pair in pair_dict): break # We can't have a dangle if it's already paired
                #if s[k]+s[k+1]+s[i]+s[i+1] in allowedPairs: changed on 10 Jan
                # this was simply incorrect
                if (t[i], t[k+1]) in allowedPairs:
                    raise Exception("that's not allowed")
                if debug_energy:
                    print("Z3 endPen {},{}:".format(i+1, k), (-1/beta)*math.log(endPen)) 
                    print("Z3 ts {},{},{},{}:".format(k, k+1, i, i+1), (-1/beta)*math.log(terminalMismatchEEnergy[t[k],t[k+1],t[i],t[i+1]]))
                return traceZbd(i+1)*terminalMismatchEEnergy[t[k],t[k+1],t[i],t[i+1]]*traceZ3d(k+2)*endPen
        ...
        if k == N-2:
            ...
            if not N-2 in dangle_list:
                if debug_energy: 
                    ...
                    print("Z3 dangle5 {},{},{}:".format(i, i+1, N-2), (-1/beta)*math.log(dangle5Energy[t[i],t[i+1],t[N-2]]))
                return traceZbd(i+1)*dangle5Energy[t[i],t[i+1],t[N-2]]*endPen

        # ts
            if N-2 in dangle_list:
                #if s[N-2]+s[N-1]+s[i]+s[i+1] in allowedPairs: # changed on 10 Jan because it didn't make sense
                if (t[i], t[N-1]) in allowedPairs:
                    raise Exception("not allowed")
                if debug_energy:
                    print("Z3 endPen {},{}:".format(i+1, N-2), (-1/beta)*math.log(endPen))
                    print("Z3 ts {},{},{}:".format(N-2, N-1, i, i+1), (-1/beta)*math.log(terminalMismatchEEnergy[t[N-2],t[N-1],t[i],t[i+1]]))
                return traceZbd(i+1)*terminalMismatchEEnergy[t[N-2],t[N-1],t[i],t[i+1]]*endPen

        # Base Case k == N-1
        # 5'D
        else:
            endPen = endPenalty.get((t[i+1],t[N-1]), 1)
            if debug_energy: 
                print("Z3 endPen {},{}:".format(i, N-1), (-1/beta)*math.log(endPen))
                print("Z3 dangle5 {},{},{}:".format(i, i+1, N-1), (-1/beta)*math.log(dangle5Energy[t[i],t[i+1],t[N-1]]))
            #print("calling on", i+1, pair_dict[i+1])
            return traceZbd(i+1)*dangle5Energy[t[i],t[i+1],t[N-1]]*endPen
...
def traceZbd(i):
    ...
    j = pair_dict[i]
    if not (t[i], t[j]) in allowedPairs:
    ...
    if case == 1 and (i+1, j-1) in pair_dict.items():
        ...
        energyZ = stackEnergy[t[i],t[i+1],t[j-1],t[j]]
    ...
    if case == 1:
        ...
        # Bulge situation
            ...
            if top > 1 or bottom > 1:
                #print(energyG)
                energyZ *= endPenalty.get((t[i],t[j]), 1)
                #print("auPen1:", endPenalty.get(s[i]+s[j], 0))
                energyZ *= endPenalty.get((t[e],t[d]), 1)
                #print("auPen2:", endPenalty.get(s[e]+s[d], 0)) 
            # Stacking energy (added 5 Dec 2016)
            if top == 1 or bottom == 1:
                energyZ *= stackEnergy[t[i],t[d],t[e],t[j]]
                #print(stackEnergy.get(s[i]+s[d]+s[e]+s[j], 0))
        ...
        # See http://rna.urmc.rochester.edu/NNDB/turner99/internal.html
            ...
            if top > 1 and bottom > 1:
                # 5' side
                energyZ *= loopFirstMismatchEnergy.get((t[i+1],t[j-1]), 1)
                #print("5' 1st", loopFirstMismatchEnergy.get(s[i+1]+s[j-1], 0))
                # 3' side
                energyZ *= loopFirstMismatchEnergy.get((t[e+1],t[d-1]), 1)
                #print("3' 1st", loopFirstMismatchEnergy.get(s[e+1]+s[d-1], 0))

            # Loop closure penalty term
            energyZ *= loopEndPenalty.get((t[i],t[j]), 1)
            energyZ *= loopEndPenalty.get((t[e],t[d]), 1)
            #print("5' AU", loopEndPenalty.get(s[i]+s[j], 0))
            #print("3' AU", loopEndPenalty.get(s[e]+s[d], 0))
                  
        if debug_energy: print("internal/bulge loop multiple of", -1/beta*np.log(energyZ))
    ...
    endPen = endPenalty.get((t[i],t[j]), 1)
    # No dangle
    ...
    if i in dangle_list and not j-1 in dangle_list:
            ...
            #print("multibranch ec:", (-1/beta)*math.log(ec))
            #print("AU penalty:", (-1/beta)*math.log(endPen))
            #print("multibranch 3dangle:", (-1/beta)*math.log(dangle3Energy[t[j],t[i],t[i+1]]))

        return (ea*eb*ec*dangle3Energy[t[j],t[i],t[i+1]]
    ...
    if not i in dangle_list and j-1 in dangle_list:
            ...
            #print("multibranch ec:", (-1/beta)*math.log(ec))
            #print("AU penalty:", (-1/beta)*math.log(endPen))
            #print("multibranch 5dangle:", (-1/beta)*math.log(dangle5Energy[t[j-1],t[j],t[i]]))

        return (ea*eb*ec*dangle5Energy[t[j-1],t[j],t[i]]
    ...
    if i in dangle_list and j-1 in dangle_list:
        ...
        #    raise Exception("this was not supposed to happen")
            ...
            #print("multibranch ec:", (-1/beta)*math.log(ec))
            #print("AU penalty:", (-1/beta)*math.log(endPen))
            #print("multibranch ts:", (-1/beta)*math.log(terminalMismatchMEnergy[t[i],t[i+1],t[j-1],t[j]]))

        return (ea*eb*eb*ec*terminalMismatchMEnergy[t[i],t[i+1],t[j-1],t[j]]
        ...
        if i in pair_dict:
            ...
            if not i-1 in dangle_list and k in dangle_list:
                ...
                scale_num = j-k
                    ...
                    #print("AU penalty:", (-1/beta)*math.log(endPen))
                    #print("multibranch eb**(j-k):", (-1/beta)*math.log(eb**(j-k)))
                    #print("multibranch 3dangle:", (-1/beta)*math.log(dangle3Energy[t[i],t[k],t[k+1]]))
                return (ec
                       * traceZbd(i)
                       * dangle3Energy[t[i],t[k],t[k+1]]
                       * (eb**(j-k))
        ...
        # i+1, k paired
            ...
            if i in dangle_list and not k in dangle_list:
                ...
                scale_num = j-k+1
                    ...
                    #print("multibranch 5dangle:", (-1/beta)*math.log(dangle5Energy[t[i],t[i+1],t[k]]))
                return (ec
                       * dangle5Energy[t[i],t[i+1],t[k]]
                       * traceZbd(i+1)
                       * (eb**(j-k+1))
            ...
            if i in dangle_list and k in dangle_list:
                ...
                # ^^ We will check if this is correct
                    ...
                    #print("AU penalty:", (-1/beta)*math.log(endPen))
                    #print("multibranch eb**(j-k+1):", (-1/beta)*math.log(eb**(j-k+1)))
                    #print("multibranch ts:", (-1/beta)*math.log(terminalMismatchMEnergy[t[k],t[k+1],t[i],t[i+1]]))
                return (ec
                       * terminalMismatchMEnergy[t[k],t[k+1],t[i],t[i+1]]
                       * traceZbd(i+1)
                       * (eb**(j-k+1))
...
def hairpinTerm(i, j):
    ...
    # There is a separate (simplified) procedure when there are only three bases in the hairpin
        ...
        energyZ *= endPenalty.get((t[i],t[j]), 1)
        #if endPenalty.get(s[i]+s[j], 0) > 0: print("    with endPen:", energy, "->", math.exp(-beta*energy))
        #print("AU:", endPenalty.get(s[i]+s[j], 0))
    ...
    # The 'usual' case, where there are at least four bases in the hairpin
        ...
        energyZ *= terminalMismatchHEnergy[t[i],t[i+1],t[j-1],t[j]]
        #if terminalMismatchHEnergy[s[i]+s[i+1]+s[j-1]+s[j]] > 0:
            #print("    with endPen:", energy, "->", math.exp(-beta*energy))

    # Add on the special GU closure energy
    # Check we don't fall off the front
    if i-2 >= 0:
        energyZ *= guClosureEnergy.get((t[i-2],t[i-1],t[i],t[j]), 1)
    ...
    #if num > 3:
        #if terminalMismatchHEnergy.get(s[i]+s[i+1]+s[j-1]+s[j], 0) != 0:
            #print("Terminal Mismatch: %s"%terminalMismatchHEnergy.get(s[i]+s[i+1]+s[j-1]+s[j], 0))
...
def multibranchTermF(i, j):
    ...
    # 3'D
    energyZ += ea*eb*ec*dangle3Energy[t[j],t[i],t[i+1]]*Z2[i+2, j-1]*endPen
    
    # 5'D
    energyZ += ea*eb*ec*dangle5Energy[t[j-1],t[j],t[i]]*Z2[i+1, j-2]*endPen
    ...
    if not (t[i+1], t[j-1]) in allowedPairs:
        ...
        energyZ += ea*eb*eb*ec*terminalMismatchMEnergy[t[i],t[i+1],t[j-1],t[j]]*Z2[i+2, j-2]*endPen
    ...
    for d in range(i+1, min(i+l,j-4)): # changed from j to j-4 (Jan 11)
        ...
        while (j-e-1) + (d-i-1) < l and eIdx >= 0:
            ...
            # 1x1 situation
                ...
                energyZ *= int11[t[i],t[i+1],t[d],t[e],t[j-1],t[j]]
                #print("int11 for ", s[i]+s[i+1]+s[d])
                #print("on        ", s[j]+s[j-1]+s[e])
                #print("is: ", int11[s[i]+s[i+1]+s[d]+s[e]+s[j-1]+s[j]])
            # 2x1 situation
            elif top == 2 and bottom == 1:
                energyZ *= int21[t[e],t[j-1],t[j],t[i],t[i+1],t[i+2],t[d]]
                #print("int21 for ", s[i]+s[i+1]+s[i+2]+s[d])
                #print("on        ", s[j]+s[j-1]+s[e])
                #print("is: ", int21[s[e]+s[j-1]+s[j]+s[i]+s[i+1]+s[i+2]+s[d]])
            # 1x2 situation
            elif top == 1 and bottom == 2:
                energyZ *= int21[t[i],t[i+1],t[d],t[e],t[j-2],t[j-1],t[j]]
                #print("int21 for ", s[i]+s[i+1]+s[d])
                #print("on        ", s[j]+s[j-1]+s[j-2]+s[e])
                #print("is: ", int21[s[i]+s[i+1]+s[d]+s[e]+s[j-2]+s[j-1]+s[j]])
            # 2x2 situation
            elif top == 2 and bottom == 2:
                energyZ *= int22[t[i],t[i+1],t[i+2],t[d],t[e],t[j-2],t[j-1],t[j]]
                #print("int22 for ", s[i]+s[i+1]+s[i+2]+s[d])
                #print("on        ", s[j]+s[j-1]+s[j-2]+s[e])
                #print("is: ", int22[s[i]+s[i+1]+s[i+2]+s[d]+s[e]+s[j-2]+s[j-1]+s[j]])
            # General case
            # See http://rna.urmc.rochester.edu/NNDB/turner99/internal.html
            else:
                ...
                if top > 1 and bottom > 1:
                    ...
                    # 3' side
                    energyZ *= loopFirstMismatchEnergy.get((t[e+1],t[d-1]), 1)
                    #print("mismatches: %s, %s"%(loopFirstMismatchEnergy.get(s[i+1]+s[j-1], 0),
                    #                            loopFirstMismatchEnergy.get(s[e+1]+s[d-1], 0)))
                
                # Loop closure penalty term
                energyZ *= loopEndPenalty.get((t[i],t[j]), 1)
                energyZ *= loopEndPenalty.get((t[e],t[d]), 1)
                #print("closures: %s, %s"%(loopEndPenalty.get(s[i]+s[j], 0),
                #                          loopEndPenalty.get(s[e]+s[d], 0)))
...
def externalLoopTerm(i, j):
    ...
    if i == N-1 and j == N:
        ...
        return 1 + dangle5Energy[t[0],t[1],t[N-1]]
    elif i == N-1:
        return Z5[j-N-1] + Z5[j-N-2]*dangle5Energy[t[j-N-1],t[j-N],t[N-1]]
    elif i == N-2 and j == N:
        return 1 + dangle3Energy[t[0],t[i],t[i+1]]
    elif j == N:
        return Z3[i+1] + Z3[i+2]*dangle3Energy[t[0],t[i],t[i+1]]
    elif i == N-2 and j == N+1:
        energyZ = 1 + dangle3Energy[t[j-N],t[i],t[i+1]] + dangle5Energy[t[j-N-1],t[j-N],t[i]]
        if not (t[i+1], t[j-N-1]) in allowedPairs:
            energyZ += terminalMismatchEEnergy[t[i],t[i+1],t[j-N-1],t[j-N]]
        return energyZ
    elif i == N-2:
        energyZ = (Z5[j-N-1]
                  + Z5[j-N-1]*dangle3Energy[t[j-N],t[i],t[i+1]]
                  + Z5[j-N-2]*dangle5Energy[t[j-N-1],t[j-N],t[i]])
        if not (t[i+1], t[j-N-1]) in allowedPairs:
            energyZ += Z5[j-N-2]*terminalMismatchEEnergy[t[i],t[i+1],t[j-N-1],t[j-N]]
        return energyZ
    elif j == N+1:
        energyZ = (Z3[i+1]
                  + Z3[i+2]*dangle3Energy[t[j-N],t[i],t[i+1]]
                  + Z3[i+1]*dangle5Energy[t[j-N-1],t[j-N],t[i]])
        if not (t[i+1], t[j-N-1]) in allowedPairs:
            energyZ += Z3[i+2]*terminalMismatchEEnergy[t[i],t[i+1],t[j-N-1],t[j-N]]
        return energyZ
    else:
        ...
        #print("i is", i)
        #print("i+2 is", i+2)
        energyZ += Z3[i+2]*Z5[j-N-1]*dangle3Energy[t[j-N],t[i],t[i+1]]
        
        # 5'D
        energyZ += Z3[i+1]*Z5[j-N-2]*dangle5Energy[t[j-N-1],t[j-N],t[i]]
        
        # tstack
        if not (t[i+1], t[j-N-1]) in allowedPairs:
            energyZ += Z3[i+2]*Z5[j-N-2]*terminalMismatchEEnergy[t[i],t[i+1],t[j-N-1],t[j-N]]
...
def stackTermW(i, j):
    ...
    # We do not need to worry about the case in which it cannot stack, as that will be taken
    # care of by the fact that in the end we multiply by Zb[i+1, j-1]
    energy = stackEnergy[t[i],t[i+1],t[j-1],t[j]]
...
def multibranchTermW(i, j):
    ...
    if i <= N-3:
        energyZ += eb*dangle3Energy[t[j],t[i],t[i+1]]*Z2[i+2, j-1]*endPen
    #if i == 6 and j == 15-N: print("3 dang energyZ", energyZ)
    
    # 5'D
    #if j >= N+2:
    if j >= 2: # changed on 19 Jan
        energyZ += eb*dangle5Energy[t[j-1],t[j],t[i]]*Z2[i+1, j-2]*endPen
    #if i == 6 and j == 15-N: print("5 dang energyZ", energyZ)
    
    # ts
    #if 1:#(t[i+1], t[j-1]) not in allowedPairs:
    #if i < N-2 and j > N+2: changed on 19 Jan
    if i <= N-3 and j >= 2:
        if not (t[i+1], t[j-1]) in allowedPairs:
            energyZ += eb*eb*terminalMismatchMEnergy[t[i],t[i+1],t[j-1],t[j]]*Z2[i+2, j-2]*endPen
...
def find_mismatches(num, length):
    for i in range(num):
        ...
        print(i, seq)
        lock_n_load(seq, zero_energies=True, wrap=False, no_dangles=False, force=True)
...
def count_structures(seq, no_dangles=False, force=False):
    seq = seq.upper()
...
def save_structure_count_list(name=0, length=20, num_samples=1000, no_dangles=False, compare=False):
    if not compare:
        x = []
        y = []
        for j in range(num_samples):
            print(j)
            x.append(length)
            y.append(count_structures(seq=generate_sequence(length), no_dangles=no_dangles))
        with open("x{}.txt".format(name), 'w') as myfile:
            for point in x:
                myfile.write(str(point)+'\n')
        with open("y{}.txt".format(name), 'w') as myfile:
            for point in y:
                myfile.write(str(point)+'\n')
    else:
        xd = []
        ...
        xn = []
        yn = []
        with open("progress2.txt", 'w') as afile:
            afile.write("current_length" + str(length)+'\n') 
        for j in range(num_samples):
            with open("progress.txt", 'a') as afile:
                afile.write("d current j " + str(j)+'\n')
            print(j)
            seq = generate_sequence(length)
            xd.append(length)
            yd.append(count_structures(seq=seq, no_dangles=False, force=True))
            xn.append(length)
            yn.append(count_structures(seq=seq, no_dangles=True, force=True))
        with open("xdangles{}.txt".format(name), 'w') as myfile:
            for point in xd:
                myfile.write(str(point)+'\n')
        with open("ydangles{}.txt".format(name), 'w') as myfile:
            for point in yd:
                myfile.write(str(point)+'\n')
        with open("xnodangles{}.txt".format(name), 'w') as myfile:
            for point in xn:
                myfile.write(str(point)+'\n')
        with open("ynodangles{}.txt".format(name), 'w') as myfile:
            for point in yn:
                myfile.write(str(point)+'\n')
...
# .txt of points
def get_thermal_ensemble(filename="myhist.txt", seq=generate_sequence(100), num_samples=5000, 
        no_dangles=False, zero_energies=False, struct_only=False, write=False, recalculate_probs=False):
    ...
    global pair_dict
    energies = []
    lock_n_load(seq, zero_energies=zero_energies, wrap=True, no_dangles=no_dangles, force=True)
    ...
    for num, i in enumerate(range(num_samples)):
        ...
        if write:
            with open("./folds/structure1.txt", 'a') as mfile:
                mfile.write(str(save_structure_count_list())+'\n')
        if struct_only:
            continue
        #print(pair_dict)
        #print(dangle_list)
...
def get_uniform_ensemble(filename="myhist.txt", seq=generate_sequence(100), num_samples=5000, 
        no_dangles=False, zero_energies=True, struct_only=False, write=False, recalculate_probs=False):
    # Save the sequence in the file
    if write:
        with open("./folds/structure1.txt", 'w') as mfile:
            mfile.write(str(seq)+'\n')
    ...
    for num, i in enumerate(range(num_samples)):
        ...
        #print(dangle_list)
        # Reset the zero so we can calculate
        all_folds.append((dangle_list, pair_dict))
        # the actual energy

        #print((-1/beta)*np.log(energy))
    lock_n_load(seq, zero_energies=False, force=True)
    #print(seq)
    #print(Z1)
    for d, p in all_folds:
        dangle_list = d
        pair_dict = p
        #print(dangle_list, pair_dict)
        energy = traceZ3d(0)
        print(-1/beta*np.log(energy))
        energies.append((-1/beta)*np.log(energy))
...
# Given that we have locked_and_loaded a sequence and have a fold
# stored in the pair_dict and dangle_list,
# return the fold in its bra-ket notation
def fold_to_bracket():
    output = ["." for i in range(N)]
    for i in range(N):
        # This could be made more efficient by extending the pair_dict to include left-hand-side also
...
# Given a dot-bracket notation string, fill the pair_dict and dangle_list
# EXAMPLE: 
# global pair_dict
# global dangle_list
# pair_dict = {}
...
def recalculate_Z(threshold=1e-8, wrap=False):
    ...
    #print("likely pairs {}".format(likely_pairs))
    #print(likely_pairs)
...
# return its energy in kcal/mol
def dot_to_energy(seq, fold):
    lock_n_load(seq)
    dot_to_dict(fold)
    energyZ = traceZ3d(0)
    return(-1/beta*math.log(energyZ))

# Given a sequence and a fold, prints out many 
# dangle-equivalent folds and their energies
def dangle_equivalent(seq, to_match, num_folds=20, num_samples=10000):
    global pair_dict
    global dangle_list
    print(seq)
    lock_n_load(seq)
    ...
    it = 0
    output = set()
    output.add((dot_to_energy(seq, to_match), to_match))
    while a < num_folds and it < num_samples:
        if it % 1000 == 0:
            print(it)
        it += 1
        pair_dict = {}
        dangle_list = []
        traceZ3(0)
        fold = fold_to_bracket()
        same = True
        for i in range(len(fold)):
            if to_match[i] == '(' or to_match[i] == ')' or fold[i] == '(' or fold[i] == ')':
                if fold[i] != to_match[i]:
                    same = False
        if same:
            energy = -1/beta*(math.log(traceZ3d(0)))
            output.add((energy, fold))
            a += 1
    output = list(output)
    output.sort(reverse=True)
    for energy,fold in output:
        print(fold, energy)
...
if __name__ == "__main__":
    ...
    #Calculate the thermal ensembles at different temperatures (for now, 37 and \infty)
    
    #seq = "UAUCAGUUUGGCGUGUGGGCUCUUAGCAUCCAUCGAUGGGUUAUGUCGUAAAAAUUGAAA"
    #seq = "CUGCUGGGCCGGCUUUACGGGAAACGGAAGGGCGAGAGGUACCGACCGCAAACUACAGGGUGGAGCUACUUGAGCCGUCCGCCCAACUUUUCGGUGGUGU"
    #seq = "AAGCAAUCCGUAUCAUAACGUACCAUGGACUUAUGGGACCUUGAGGGCCG"
    #get_thermal_ensemble(filename="i2.txt", seq=seq, num_samples=10000, 
       #no_dangles=False, zero_energies=False, struct_only=False, write=True, recalculate_probs=False)
    #get_uniform_ensemble(filename="i2.txt", seq=seq, num_samples=10000, 
    #    no_dangles=False, zero_energies=True, struct_only=False, write=False, recalculate_probs=False)
    ...
    #seq = "CCUUCAAGAAG"
    #lock_n_load(seq)
    #pair_dict = {1:10, 2:8, 3:7}
    #dangle_list = [0]
    #print(-1/beta*np.log(traceZ3d(0)))

    print(dot_to_energy("GGGAAACCC", "(((...)))"))
    
    # Compare thermal ensembles of structures with and without dangles
    '''
    for length in [200, 250, 300]:
        for i in range(20):
            seq = generate_sequence(length)
            print(seq)
            get_thermal_ensemble(filename="bigthermaldangles{}.txt".format(length), seq=seq, num_samples=5000, 
                no_dangles=False, zero_energies=False, struct_only=False, write=False, recalculate_probs=True)
            get_thermal_ensemble(filename="bigthermalnodangles{}.txt".format(length), seq=seq, num_samples=5000, 
                no_dangles=True, zero_energies=False, struct_only=False, write=False, recalculate_probs=True)
    with open("lengths.txt", 'w') as afile:
        for length in [50, 100, 150, 200, 250, 300]:
            for i in range(20):
                afile.write(str(length)+'\n')
    '''
    
    #for i in [100, 200, 300, 400]:
        #seq = generate_sequence(i)
        #save_structure_count_list(name=str(i),   no_dangles=False, length=i, num_samples=2000, compare=True)
    ...
    #for length in [100, 200, 300]:
        ...
        #for seq_number in range(5):
            #seq = generate_sequence(length)
            #lock_n_load(sequence=seq, wrap=False)
            #output += seq + " " + str(-1/beta*math.log(Z3[0])) + '\n'
            #for i in range(int(1e5)):
                #pair_dict = {}
                #dangle_list = []
                #traceZ3(0)
                #output += str(round(-1/beta*(math.log(traceZ3d(0))), 2)) + '\n'
                #if i % 1000 == 0:
                    #print(i)
    ...
    global pair_dict
    global dangle_list
    output = "n lock_n_load recalculate_Z traceZ3\n"
    for n in [50, 100, 150]:#, 200, 250, 300]:
        ...
        for i in range(10):
            ...
            #First load
            t0 = time.time()
            lock_n_load(generate_sequence(n), zero_energies=False, wrap=True)
            t1 = time.time()
            output += str(t1-t0) + " "
    ...
    with open("timings.txt", 'w') as afile:
        afile.write(output)
    '''

    
    # Calculates the difference in energy of the usual partition function
    # and the re-calculated one after thresholding for likely pairs
    '''
    global pair_dict
    global dangle_list
    with open("diffenergy.txt",'w') as afile:
        afile.write("thresh,length,diff\n")
    sequences = {}
    for i in [50, 100, 150, 200, 250]:
        sequences[i] = []
        for j in range(10):
            sequences[i].append(generate_sequence(i))
    # sequences now countains 10 sequences
    # of each length
    print(sequences)
    for thresh in [1e-12, 1e-10, 1e-8, 1e-6, 1e-4, 1e-2]:
        print("thershold currently", thresh)
        for i in [50, 100, 150, 200, 250]:
            for j in range(10):
                seq = sequences[i][j]
                pair_dict = {}
                ...
                totful = 0
                totles = 0
                lock_n_load(seq, wrap=True)
                totful = Z3[0]
                recalculate_Z(threshold=thresh)
                totles = Z3[0]
                #print(thresh,i,-1/beta*np.log(totles)+1/beta*np.log(totful))
                with open("diffenergy.txt",'a') as afile:
                    afile.write(str(thresh)+','+str(i)+','+str(-1/beta*np.log(totles)+1/beta*np.log(totful))+'\n')
            #print("full", totful)
            #print("new", totles)
            #print("abs difference in kcal/mol", -1/beta*np.log(totles)+1/beta*np.log(totful))
                #print("rel difference", (full-less)/full) 
            #absdif[-1] = (-1/beta*np.log(totful)+1/beta*np.log(totles))
            #reldif[-1] = (totful-totles)/totful
            #print("thresh", thresh)
            #print("length {}:".format(i), "abs", absdif[-1])
            #print("length {}:".format(i), "rel", reldif[-1])
    '''
    

    '''
    # Calculates the partition function and outputs pij > theta
    #global pair_dict
    #global dangle_list
    with open("pij3.txt",'w') as afile:
        afile.write("thresh,len,numgreater\n")
    lengths = []
    sequences = {}
    for i in [50, 100, 150, 200, 250]:
        sequences[i] = []
        for j in range(10):
            sequences[i].append(generate_sequence(i))
    for thresh in [0, 1e-10, 1e-8, 1e-6, 1e-4, 1e-2]:
        print("thresh currently",thresh)
        for i in [50, 100, 150, 200, 250]:
            for j in range(10):
                seq = sequences[i][j]
                pair_dict = {}
                ...
                for a in range(N):
                    for b in range(N):
                        if Zb[a,b]*Zb[b,a]/Z3[0] > thresh:
                            numgreater += 1
                with open("pij3.txt",'a') as afile:
                    afile.write(str(thresh)+','+str(i)+','+str(numgreater/N)+'\n')
    '''
    ...
    #for n in [50, 100, 150, 200, 250, 300]:
        ...
        #for i in range(10):
            ...
            #t1 = time.time()
            #output += "{} {}\n".format(n, t1 - t0)
    #with open("fast.txt", 'w') as afile:
        #afile.write(output)

    #print("globals")
    #for x in sorted(globals()):
        #print(x)
    #folds_given_length(filename="length20.txt", length=20, num_samples=1000)
    #folds_given_length(filename="length300_3.txt", length=300, num_samples = 1000)
    #get_thermal_ensemble(seq=generate_sequence(14), num_samples=1000)
    #save_structure_count_list(number=7, no_dangles=True)
